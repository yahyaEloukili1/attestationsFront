...
export class DecheiraComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    ...
    this.mapService.getCommunes().subscribe((data: FeatureCollection) => {
      ...
      const dcheiraFC: FeatureCollection = { type: 'FeatureCollection', features };

      // Draw commune polygon
      const dcheiraLayer = L.geoJSON(dcheiraFC as any, {
        style: {
          color: '#263238',
          weight: 3,
          fillColor: '#f2b6b6',
      ...
      const leafLayer: any = dcheiraLayer.getLayers()[0];
