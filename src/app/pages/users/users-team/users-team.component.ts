...
@Component({
  ...
  styleUrl: './users-team.component.scss'
})
...
users
  constructor(private formBuilder: UntypedFormBuilder,private router: Router,
     private modalService: NgbModal, private offcanvasService: NgbOffcanvas,public restApi: restApiService) { }
...
this.getReources()
  ...
  getReources(){
    this.restApi.getResourceAll('appUsers').subscribe(data=>{
  ...
  saveTeam() {
      if (this.teamForm.valid) {
        if (this.teamForm.get('_id')?.value) {
          this.Team = this.Team.map((order: { id: any; }) => order.id === this.teamForm.get('_id')?.value ? { ...order, ...this.teamForm.value } : order);
