...
export class FoumelouadComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    ...
    const target = normalize('FOUM EL OUED');
    ...
    this.mapService.getCommunes().subscribe((data: FeatureCollection) => {
      ...
      const foumFC: FeatureCollection = { type: 'FeatureCollection', features };

      // Draw commune polygon
      const foumLayer = L.geoJSON(foumFC as any, {
        style: {
          color: '#263238',
          weight: 3,
          fillColor: '#c7e3c1',
  ...
  private loadAllProjects() {
    ...
    (this.mapService as any).getEauPotable7?.().subscribe((fc: any) =>
      this.buildFilteredPolygonLayer(fc, 'P1 – Réhabilitation du réseau AEP', '#ef4444', (n) => (this.eauP1Count = n)).addTo(this.eauP1Layer)
    );
    ...
    (this.mapService as any).getEauPotable5?.().subscribe((fc: any) =>
      this.buildFilteredPolygonLayer(fc, 'P3 – STEP – Zone Industrielle El Marsa', '#ef4444', (n) => (this.eauP3Count = n)).addTo(this.eauP3Layer)
    );
    (this.mapService as any).getEauPotable4?.().subscribe((fc: any) =>
      this.buildFilteredPolygonLayer(fc, 'P4 – Assainissement – Laayoune', '#ef4444', (n) => (this.eauP4Count = n)).addTo(this.eauP4Layer)
    );
    ...
    (this.mapService as any).getEauPotable2?.().subscribe((fc: any) =>
      this.buildFilteredPolygonLayer(fc, 'P6 – Assainissement – Foum El Oued', '#ef4444', (n) => (this.eauP6Count = n)).addTo(this.eauP6Layer)
    );
    (this.mapService as any).getEauPotable?.().subscribe((fc: any) =>
      this.buildFilteredPolygonLayer(fc, 'P7 – Extension de la station de dessalement', '#ef4444', (n) => (this.eauP7Count = n)).addTo(this.eauP7Layer)
    );
