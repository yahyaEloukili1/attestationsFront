...
export class LaayouneComponent implements AfterViewInit, OnDestroy {
  ...
  // ✅ Show project buttons on the right (same as provinceLaayoune)
  showSanteActive = false;
  showEmploiActive = false;
  showEauActive = false;

  // ✅ Counts for project buttons
  ...
  // =========================
  // ✅ Toggle modes (same as provinceLaayoune)
  // =========================
  ...
  private loadAllProjects() {
    ...
    this.mapService.getCHRs().subscribe((fc: any) => {
      this.santeNouveauxCount += this.addFilteredPointsToLayer(
        fc,
        this.santeNouveauxLayer,
        this.hopitauxExistantsIcon,
        'Extension CHR'
      );
    });
    ...
    (this.mapService as any).getZoneDouirat?.().subscribe((fc: any) => {
      this.buildFilteredPolygonLayer(fc, 'Zone Douirat', '#22c55e', (n) => (this.santeDouiratCount = n))
        .addTo(this.santeDouiratLayer);
    });
